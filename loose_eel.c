...
int runcode(NSEEL_VMCTX m_vm, const char *codeptr, int showerr, const char *showerrfn, int canfree, int ignoreEndOfInputChk, int doExec)
{
...
int loadfile(NSEEL_VMCTX m_vm, const char *fn, const char *callerfn, int allowstdin)
{
	FILE *fp = fopen(fn, "r");
	if (!fp)
	{
	...
	int ret = runcode(m_vm, codetext, callerfn ? 2 : 1, fn, 1, 1, !callerfn);
	s_str_destroy(&code);
	return ret;
}
int main(int argc, char **argv)
{
	...
	if (g_interactive)
	{
		...
		EEL_F *resultVar = NSEEL_VM_regvar(m_vm, "__result");
		s_str_clear(&code);
		s_str_clear(&t);
		char line[4096];
		for (;;)
		{
			...
			int res = runcode(m_vm, tget, 0, "", 1, 1, 1); // allow free, since functions can't be defined locally
			if (!res)
			{
				if (resultVar) printf("=%.17g\n", *resultVar);
				s_str_assign_c_str(&code, "");
			}
